...
总结：
Mongodb暂且不支持 WSL [相关说明链接](https://docs.mongodb.com/manual/administration/install-on-linux/)

暂且使用Windows环境下做测试：

cd E:\DB\MongoDB\Server\3.2\bin

mongod --port 27017 --storageEngine wiredTiger --dbpath E:\DB\MongoDB\CodeSnippet\DataBase --rest --nojournal >> E:\DB\MongoDB\CodeSnippet\DataBase\Logger.log

[Windows引导安装方式安装包](https://fastdl.mongodb.org/win32/mongodb-win32-x86_64-2008plus-ssl-4.0.6-signed.msi)

测试代码：

    mongo localhost:27017 #客端连接

    use test    #切换数据库

    db.mytest.find() #简单数据查询